...
Latest release: **0.3.3**, carrying driver **20.47.10.679**
(`nabu-ln8000-driver-0.3.3-arm64.zip`, produced by the `release` workflow from the commit
tagged `v0.3.3`, and installed and read back on the development tablet before it was
published). It adds the `PublishBattery` switch for a machine that publishes a battery of
its own, makes `bring-up.ps1` report the PMIC layer it runs on (the fingerprint of a
community `usbfix` package) and writes down what a patched `qcpmicext8150.sys` means for
fast charging. The development tablet now runs **20.47.10.679** (`oem173.inf`, device node
`OK` / `CM_PROB_NONE`) - the number is per install, `uninstall-driver.ps1` prints the ones
it removes.

The previous release, **0.3.2** (driver `20.47.10.677`), carries the charging fixes
described below: the transfer band and the QC3 target come from the fuel gauge instead of
a pump channel that mirrors the bus, a running transfer is neither pulsed down nor
switched off on a sampled verdict, and 1:1 is a five-volt mode again.

The releases before it: **0.3.1** (driver 20.47.10.672) was built and measured by hand
before this repository was made public - a Quick Charge brick negotiated to 8.3 V, `Iin`
0.23-1.28 A, SoC 196 -> 199, the charging flag 338 ms after the verdict, the unplug
released in 1.1 s - and **0.3.0** (driver 20.47.10.665) introduced the access policy: a
process that is not elevated is refused with `ERROR_ACCESS_DENIED` (5).
...
0.3.2 (driver `20.47.10.677`): the transfer band and the QC3 target now come from the fuel
...
backlight reset is fixed in 0.3.2 (driver `20.47.10.677`): a tick that reads only the pump's
...
| 💡 AC-verdict flap / backlight reset | Fixed in 0.3.2 (driver `20.47.10.677`). The 22.09 capture of the flap is in `docs/FINDINGS.md`; the 24.09 build was confirmed live — one no-verdict tick on the pull, DC 1.0 s after the cable, AC in the first sample after the replug | ⚠️ |
...
**Fixed in 0.3.2** (driver `20.47.10.677`, `oem171.inf`): a tick that reads
...
and with the adapter off as well as on — and is listed on its own.

| Defect | What it does |
|---|---|
| AC arrives seconds after the cable | The verdict (`OnlineRaw`) is on the first tick, but Windows reads the *next* one and the pump bring-up blocks the tick: measured **8.7 s** from insertion to `pwr = 1` on a Quick Charge brick, 5.6 s on a plain 5 V one. **Fixed in 0.3.2** (`20.47.10.677`): the online hold is front-armed, and the 24.09 replug carried the flag in its first sample |
