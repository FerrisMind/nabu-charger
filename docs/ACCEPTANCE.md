...
Rust 1.97.0 (`rust-toolchain.toml` pins the channel), WDK 10.0.26100. The host checks
above need no LLVM; the kernel drivers do, and they need **17.0.6** - on 23.x `bindgen`
fails to parse the WDF headers (criterion 20 below, [HANDOVER.md](HANDOVER.md)).
...
| 20 | A reproducible signed artifact with a checksum | <span>done</span> | `deploy/build-arm64.ps1`: both drivers, signing, `artifacts/SHA256SUMS.txt` (19 records), `artifacts/BUILD-MANIFEST.json` with the versions (rustc 1.97.0, cargo-wdk 0.1.1, WDK 26100, LLVM bin). **Reproducibility is not declared, it is measured**: the pipeline builds twice - before the second build it deletes the crate's `target\aarch64-pc-windows-msvc\release` and aborts unless that build prints `Compiling <crate>`, because a cached second build witnesses nothing - and writes the verdict into the manifest (`reproducible`). The comparison ignores the Authenticode table and the PE checksum and nothing else, so every section of the image is compared: they are identical between builds and between the released package and the binary installed on the tablet (`true`), and a code or flag change makes the image itself differ, which the note reports as `NOT reproducible` |
