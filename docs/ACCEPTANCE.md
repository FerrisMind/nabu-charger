...
| 14 | Ядро драйвера charge pump LN8000 | <span>выполнено</span> | `crates/ln8000`: карта регистров, режимы, формулы тока/напряжения, разбор защит и АЦП; 29 тестов + doctests; собирается без `std` (`cargo build -p ln8000 --no-default-features`); демо — `artifacts/verify-pump.txt`; логика сверена с GPL-драйвером Android, детали — `docs/LN8000.md` |
