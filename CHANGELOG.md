...
* **A removal took 7-8 s to reach Windows, and every second of it was the hold.** With the
  ...
  Measured on the released 20.47.10.672 with the operator's brick: the first tick after the
  pull (the reflection `Vin = 8 144 000` against a 4 105 000 cell, `FAULT1` bit 4 still clear)
  ended the window through the doubling veto, the hardware bit asserted 283 ms later, and the
  three-tick rule released it 1.1 s after the cable came out.
...
* **The device object was unnamed, so the access-policy descriptor was rejected and the
  device failed to start.** `WdfDeviceInitAssignSDDLString` requires a named device object,
  and a PnP device must not name its own: the documented form is
  `WdfDeviceInitSetCharacteristics` with `FILE_AUTOGENERATED_DEVICE_NAME`, which is what the
  driver does now. The mistake was invisible where it was made - the assignment returned
  `STATUS_SUCCESS` and the descriptor was rejected later, inside `WdfDeviceCreate`, as
  `STATUS_INVALID_SECURITY_DESCR` (`0xC0000079`), after which the device failed to start with
  `CM_PROB_FAILED_ADD` (31). Measured on the tablet with 20.47.10.664: the `SddlSt` mark read
  0 while the driver stopped at `STAGE_DEVICE`.

* **Every command of the diagnostic tool died before its body ran.** `Invoke-DeviceIoControl`
  took its buffer as `[byte[]]$Input`, and PowerShell reserves `$Input` for the incoming
  pipeline, so the binding failed with "cannot convert ...ArrayListEnumeratorSimple... to
  System.Byte[]". Two `Marshal` calls in the same script had the same class of problem:
  `SizeOf($Type)` and `PtrToStructure($ptr, $Type)` make the binder pick the `object`
  overload and then fail to marshal the `RuntimeType` itself. The parameter is `$Buffer` now,
  the size comes from an instance, and the structure is read through the generic overload by
  reflection.

### Changed

* **The release archive verifies and unpacks on any platform.** `Set-Content` wrote the
  checksum manifests with CRLF, which made `sha256sum -c` fail on every line (0 of 22 OK),
  and `Compress-Archive` wrote ZIP entry names with backslashes, which made `unzip` refuse
  the archive on Linux and macOS ("appears to use backslashes as path separators"). The
  manifests are written with LF endings, the entries are added by hand with forward slashes,
  and `install-driver.ps1` now refuses to install with Secure Boot on, which `testsigning`
  does not override. Measured on the 0.3.1 archive: `unzip -l` clean, `sha256sum -c` 22 of
  22 OK.
